...
void ChangeColor(){ // 副程式使led動作, 轉變顏色 // 因為要讓顏色從light1變成light2，因此讓light1的質漸漸變成0且讓light2的質漸漸變成light2指定之數值
	for(counter=1;counter<=(TurnTime/12);counter++){
		...
		if(map(counter,0,TurnTime/12,0,light2[0])!=0){
			analogWrite(Red,map(counter,0,TurnTime/12,0,light2[0])); // light2的Red漸漸變成light2指定的Red
			delay(2);
		}
		if(map(TurnTime/12-counter,0,TurnTime/12,0,light1[1])!=0){
			analogWrite(Green,map(TurnTime/12-counter,0,TurnTime/12,0,light1[1])); // light1的Green漸漸變成0
			delay(2);
		} 
		if(map(counter,0,TurnTime/12,0,light2[2])!=0){ 
			analogWrite(Blue,map(counter,0,TurnTime/12,0,light2[2])); // light2的Blue漸漸變成light2指定的Blue
			delay(2);
		} 
		if(map(TurnTime/12-counter,0,TurnTime/12,0,light1[0])!=0){
			analogWrite(Red,map(TurnTime/12-counter,0,TurnTime/12,0,light1[0]));   // light1的Red漸漸變成0
			delay(2);
		} 
		if(map(counter,0,TurnTime/12,0,light2[1])!=0){ 
			analogWrite(Green,map(counter,0,TurnTime/12,0,light2[1])); // light2的Green漸漸變成light2指定的Green
			delay(2); 
		} 
		if(map(TurnTime/12-counter,0,TurnTime/12,0,light1[2])!=0){ 
			analogWrite(Blue,map(TurnTime/12-counter,0,TurnTime/12,0,light1[2]));  // light1的Blue漸漸變成0
			delay(2);
		}
	}
	for(counter=1;counter<=MaintainTime/6;counter++){ 
		analogWrite(Red,light2[0]);   // 將Red參數指定為light2的Red
		delay(2);
		analogWrite(Green,light2[1]); // 將Green參數指定為light2的Green
		delay(2);
		analogWrite(Blue,light2[2]);  // 將Blue參數指定為light2的Blue
		delay(2);
	}
}